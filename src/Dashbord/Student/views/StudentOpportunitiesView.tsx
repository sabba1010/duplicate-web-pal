      ...
      <div className="p-6 border-b border-slate-100 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="p-2.5 rounded-xl border border-pink-200 text-[#e04f96] hover:bg-pink-50 transition-colors flex items-center gap-2">
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`whitespace-nowrap px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${
                activeCategory === cat
                  ? "bg-[#e04f96] text-white shadow-sm shadow-pink-200/50"
                  : "bg-pink-50 text-gray-500 hover:bg-pink-100 hover:text-[#e04f96]"
              }`}
            >
              {cat}
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {RECOMMENDED_OPPORTUNITIES.map((opp, i) => (
            <motion.div 
              ...
              className="bg-white rounded-2xl border border-pink-100 shadow-sm overflow-hidden hover:shadow-md hover:border-pink-200 transition-all group"
            >
              <div className="h-40 relative overflow-hidden">
                ...
                <div className="absolute top-3 left-3 bg-white/90 backdrop-blur-sm px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider text-[#e04f96] shadow-sm">
                  {opp.type}
                </div>
              ...
              <div className="p-5">
                <h3 className="font-bold text-[#2d1b28] text-base leading-tight mb-1 group-hover:text-[#e04f96] transition-colors line-clamp-2">
                  {opp.title}
                </h3>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {opp.tags.slice(0, 3).map(tag => (
                    <span key={tag} className="bg-pink-50 text-[#e04f96] text-[10px] font-semibold px-2 py-0.5 rounded-full">
                      {tag}
                    </span>
                  ))}
                </div>
