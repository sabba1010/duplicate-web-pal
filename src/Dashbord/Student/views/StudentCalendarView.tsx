    ...
    <div className="bg-white rounded-3xl border border-slate-200 shadow-sm h-full flex flex-col overflow-hidden">
      <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 bg-blue-50/50 border border-blue-100 px-3 py-1.5 rounded-xl">
          ...
          <span className="text-xs font-semibold text-blue-700">Google Calendar Synced</span>
        </div>
      </div>

      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        {/* Main Calendar Area (Placeholder for actual grid) */}
        <div className="flex-1 p-6 border-r border-slate-100 bg-slate-50/50 overflow-y-auto">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="grid grid-cols-7 auto-rows-[100px] divide-x divide-y divide-slate-100">
              ...
              {Array.from({ length: 35 }).map((_, i) => (
                <div key={i} className={`p-2 relative ${i === 24 ? 'bg-teal-50/30' : ''}`}>
                  <span className={`text-sm font-semibold ${i === 24 ? 'text-teal-600' : 'text-slate-400'}`}>
                    {(i % 31) + 1}
                  </span>
                  {/* Mock Event Dot */}
                  {[12, 15, 24, 28].includes(i) && (
                    <div className="absolute bottom-2 left-2 right-2 h-1.5 rounded-full bg-teal-400"></div>
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="w-full lg:w-80 p-6 bg-white overflow-y-auto">
          <h3 className="font-bold text-slate-900 mb-4 flex items-center gap-2">
            <CalendarIcon className="h-4 w-4 text-teal-600" /> Upcoming
          </h3>
          <div className="space-y-4">
            {CALENDAR_EVENTS.map(event => (
              <div key={event.id} className="p-4 rounded-2xl border border-slate-100 bg-slate-50 hover:border-teal-200 transition-colors group">
