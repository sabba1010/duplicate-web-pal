  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200 shadow-sm h-full flex flex-col overflow-hidden">
      <div className="p-6 border-b border-slate-100 flex items-center justify-between">
        ...
        <button className="bg-slate-900 hover:bg-slate-800 text-white px-4 py-2 rounded-xl text-sm font-semibold shadow-sm transition-colors flex items-center gap-2">
        ...
        <div className="flex gap-6 min-w-max h-full">
          {COLUMNS.map((col) => (
            <div
              ...
              className="w-80 flex flex-col bg-slate-100/50 rounded-2xl p-4 border border-slate-200/50 h-full"
            >
                ...
                <AnimatePresence>
                  {items.filter(i => i.status === col).map((item) => (
                    <motion.div
                      ...
                      className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm cursor-grab active:cursor-grabbing hover:shadow-md hover:border-teal-200 transition-all group"
                    >
